...
def emails_menu():
    keyboard = [
        ...
        [InlineKeyboardButton("🗑️ Delete Email", callback_data="btn_email_delete")],
...
def delete_email_menu(emails: list[str]):
    keyboard = []
    for idx, email in enumerate(emails):
        label = email if len(email) <= 30 else email[:27] + "..."
        keyboard.append([InlineKeyboardButton(f"❌ {label}", callback_data=f"email_del_{idx}")])
...
def control_menu(token: str):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🗑️ Remove Account", callback_data=f"ctrl_del_{token_short}")],
